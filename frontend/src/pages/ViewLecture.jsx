...
function ViewLecture() {
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6 flex flex-col md:flex-row gap-6">
      ...
      <div className="w-full md:w-2/3 bg-white rounded-2xl shadow-md p-6 border border-gray-200">
        ...
        <div className="mb-6" >
          ...
          <div className="mt-2 flex gap-4 text-sm text-gray-500 font-medium">
            <span>{t('category')}: {selectedCourse?.category}</span>
            <span>{t('course_level')}: {selectedCourse?.level}</span>
          </div>
        </div>
      ...
      <div className="w-full md:w-1/3 bg-white rounded-2xl shadow-md p-6 border border-gray-200 h-fit">
        <h2 className="text-xl font-bold mb-4 text-gray-800">{t('all_lectures')}</h2>
        <div className="flex flex-col gap-3 mb-6">
          {selectedCourse?.lectures?.length > 0 ? (
            ...
            <p className="text-gray-500">{t('no_lectures_available')}</p>
          )}
        </div>
  ...
  <div className="mt-4 border-t pt-4">
    <h3 className="text-md font-semibold text-gray-700 mb-3">{t('instructor')}</h3>
    <div className="flex items-center gap-4">
      ...
      <div>
        ...
        <p className="text-sm text-gray-600">
          {courseCreator.description || t('no_bio_available')}
        </p>
      </div>
    </div>
  </div>
